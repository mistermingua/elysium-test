...
await page.waitForTimeout(1000);    
  ...
  await page.locator('.k-alt > td:nth-child(3)').first().click();
  await page.locator('.editablefield').first().click();
  await page.getByRole('option', { name: 'Vacation' }).click();
  await page.getByRole('gridcell', { name: '0', exact: true }).click();
  await page.locator('tr:nth-child(3) > td:nth-child(3)').first().click();
  await page.getByRole('option', { name: 'Vacation' }).click();
  await page.getByRole('gridcell', { name: '0', exact: true }).nth(1).click();
  await page.locator('#PaycodeAmount').fill('8');
  await page.locator('#PaycodeAmount').press('Tab');
