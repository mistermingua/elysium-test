...
test('Creacion de Payroll Extraordinary contando con Christmas Bonus y Performance Bonus'
    , async ({ page }) => {
...
await page.waitForTimeout(2000);
  ...
  await page.getByRole('option', { name: 'Extraordinary' }).click();
  await page.getByRole('listbox', { name: 'Class' }).getByLabel('select').locator('span').click();
  
  // Descomentar la clase de nómina que se quiera crear
  await page.getByRole('option', { name: 'Christmas Bonus' }).click(); 
  //await page.getByRole('option', { name: 'Performance Bonus' }).click();
  ...
  await page.getByRole('option', { name: 'Current Employee Selection' }).click();
