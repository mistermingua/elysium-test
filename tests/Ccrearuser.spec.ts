...
test('test', async ({ page }) => {
  ...
  await page.locator('#txtRehireSearchSsn').fill(user.ssn); // El SSN sera la base de busqueda del usuario creado
  await page.getByRole('button', { name: ' Search' }).click(); // Busca el usuario por SSN
    await page.waitForTimeout(2000);
  await page.getByRole('button', { name: '' }).click(); // Selecciona el usuario encontrado por SSN
    await page.waitForTimeout(1000);
  await page.getByText('Position', { exact: true }).click();
   await page.waitForTimeout(1000);
  await page.getByText('Time and Attendance').click();
   await page.waitForTimeout(1000);
  await page.getByText('Payroll', { exact: true }).click();
   await page.waitForTimeout(1000);
  await page.locator('span').filter({ hasText: 'Attributes' }).click();
   await page.waitForTimeout(1000);
  ...
  await page.getByText('Notes').click();
   await page.waitForTimeout(1000);
  await page.getByText('Pay History').click();
   await page.waitForTimeout(1000);
  await page.getByText('Files').click();
   await page.waitForTimeout(1000);
  await page.getByText('Supervisor Profile').click();
   await page.waitForTimeout(1000);
await page.locator('#tabstripEmployee').getByText('Evaluations', { exact: true }).click();
   await page.waitForTimeout(1000);
