...
// Si
const alerta = await page.getByRole('dialog', { name: 'Employee was successfully' });// alerta sera el dialogo que se muestra cuando se crea un usuario correctamente
  
// En el caso de que alerta en algun monento se muestre el test fallará y mostrará un mensaje de error codigo abajo
if (await alerta.isVisible().catch(() => false)) {
  throw new Error('❌ Error: Se mostró una alerta, el test falla aquí.');
} // Si la alerta es visible, lanza un error para detener la prueba y mostrar el mensaje de error


// Si lo de arriba no se cumple en ningun mometno el test continuara normalmente y caundo muestre por segunda vez el bloque ssn se cumplira
...
//await page.waitForTimeout(1500); 
  ...
  // Poner aqui el numero de seguro social del usuario que se va a crear en este caso sera repetido para provocar el error en la pagina
  // 
...
await page.waitForTimeout(3000); // Esperar 3 segundos para que se cargue el perfil del usuario creado 

  // await page.getByRole('dialog', { name: 'Alert' }).locator('div').nth(2).click();  //  Esto es del cuadro de dialogo que se muestra cuando se crea un usuario correctamente




// Volvemos a intentar crear el usuario con los mismos datos para provocar el error de usuario repetido
// El siguientre usuario dara error por tener el mismo SSN, numero de placa y numero de identificacion que el usuario creado anteriormente
...
await page.waitForSelector(`role=option[name="${user.costCenter}"]`, { state: 'visible'});
  ...
  // Espera a que aparezca el mensaje de alerta y luego cierra el diálogo, este mensaje mostrara que no se puede crear un usuario con el mismo ssn, username, id y badge
  await page.getByRole('link', { name: ' OK' }).click();


// Si el boton de arriba de presiona correctamente el test se considera exitoso, ya que se ha comprobado que no se puede crear un usuario con los mismos datos
// Si el boton de arriba no se presiona el test fallara y mostrara un mensaje
...
//Hacer que el test se realize una vez , para que no se creen usuarios repetidos.
//Y que el test sea exitoso si no se crea 
//
//ssn, username , id y badge
